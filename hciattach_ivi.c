...
void timb_unistone_reset(int fd)
{
	struct timespec tm = { 0, 500000000 };
	struct timeval tv = { 1, 0 };
	fd_set watch;
	unsigned char cmd_rst[10];
	int retval;

	cmd_rst[0] = HCI_COMMAND_PKT; // Determine what module we're talking to
	cmd_rst[1] = 0x03;
	cmd_rst[2] = 0x0c;
	cmd_rst[3] = 0x00;
	...
	nanosleep(&tm, NULL);
	/* Send RESET command */
	if (write(fd, cmd_rst, 4) != 4) {
		perror("Can't write HCI_Reset.");
	}
	nanosleep(&tm, NULL);
	tcflush(fd, TCIOFLUSH);
	FD_ZERO(&watch);
...
static int timb_unistone(int fd, struct uart_t *u, struct termios *ti)
{
	fd_set watch;
	struct timeval tv = { 1, 0 };

	unsigned char cmd[10], cmd_rst[10], resp[HCI_MAX_EVENT_SIZE];
	int baudrates[] = { u->init_speed, 921600, 115200, 57600 };
	int old_alarm = alarm(20); // Extend timeout
	int i, retval;

	timb_unistone_reset(fd);
...
struct uart_t uart[] = {
	...
	{ "timb-unistone",0x0000,0x0000,HCI_UART_H4,   115200, 921600, FLOW_CTL, NULL, timb_unistone },
